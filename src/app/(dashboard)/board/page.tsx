  ...
  const widgets = [
    {
      ...
      content: (
        <div className="flex flex-col h-full">
          ...
          >
            <div className="flex gap-4 overflow-x-auto pb-4 p-3 flex-1 w-full" style={{ minHeight: "200px" }}>
          ...
          <div className="border-t border-slate-200 shrink-0">
            <button
              ...
              className="flex items-center justify-between w-full px-4 py-2 hover:bg-slate-50 transition-colors"
            >
              <div className="flex items-center gap-2">
                <Archive className="h-3.5 w-3.5 text-slate-400" />
                <span className="text-xs font-medium text-slate-600">Archives</span>
              ...
              <ChevronDown className={cn("h-3.5 w-3.5 text-slate-400 transition-transform", showArchiveSection && "rotate-180")} />
            </button>
            {showArchiveSection && (
              <div className="px-3 pb-3">
                {archiveLoading ? (
                  ...
                  <p className="text-xs text-slate-400 text-center py-3">Aucune carte archivée</p>
                ) : (
                  <div className="max-h-52 overflow-y-auto space-y-1.5 pr-1">
                    {archivedCards.map((card) => (
                      <div
                        key={card.id}
                        className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors group cursor-pointer"
                      ...
                      >
                        ...
                        <span className="text-[10px] text-slate-400 flex-shrink-0 hidden sm:block">
                          {card.archivedAt
                            ? new Date(card.archivedAt).toLocaleDateString("fr-FR", { day: "2-digit", month: "short", year: "numeric" })
                            : ""}
                        </span>
                        {card.tags?.slice(0, 2).map((t) => (
                          <span
                            key={t.id}
                            className="hidden md:inline-block px-1.5 py-0.5 text-[9px] font-medium rounded flex-shrink-0"
